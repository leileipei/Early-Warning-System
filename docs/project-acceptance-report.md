...
**验收日期：** 2026-07-17
**验收提交：** `aef3fe5 fix: fail closed on ambiguous secret mappings`
**结论：** 完成并通过验收。
...
| `pytest --cov=app --cov-report=term-missing --cov-fail-under=93` | `519 passed in 22.31s`；总覆盖率 `94.11%`（门槛 93%） |
| `pip check` | 无破损依赖 |
...
分支为 `codex/production-readiness` 到 `main`。HEAD `aef3fe5` 的 push 与 PR 两组 CI 均
完成：Python 3.11、3.12、3.13 共六个 job 均通过。GitHub Actions run IDs：
`29583486029`、`29583490199`。
...
- `/health` 返回 200；Worker 运行时 `/health/ready` 返回 200；停止 Worker 超过 3 秒后
  `/health/ready` 返回 503，组件状态为 `worker=stale`。最终提交 `aef3fe5` 再复验同样
  得到 200/503 行为。
...
- `/tmp/ews-acceptance/logs-mobile-390x844-clean.jpg`
...
验收后复审发现的文档和截图问题（2 个 Important、3 个 Minor）均由
`419d935 fix: close final acceptance review gaps` 关闭：该提交修正嵌套结构脱敏、
`operations.md` 停机/备份顺序和连续编号，以及 `deployment.md` 1-22 章节号；移动日志
截图更新为干净版本。

随后 `a886984 fix: redact multiline structured secrets` 支持格式化多行 JSON 与键冒号换行
的脱敏；`aef3fe5 fix: fail closed on ambiguous secret mappings` 对未闭合、65 层深度和
32768 扫描窗口歧义统一采取 fail-closed 行为。跨行与 fail-closed 边界均已关闭。

最终独立复审范围 `a886984..aef3fe5` 的 Critical、Important、Minor 均为 0，结论为
`Ready=yes`；定点 `tests/test_error_reporting.py` 为 `22 passed`。
